...
def getList():
    ...
    # if ticker_list has only 1 ticker, return error
    if len(ticker_list) <= 1:
        return jsonify({
            ...
            'error': {
                "code": "400",
                "name": "Invalid input",
                "description": "There must be at least two tickers"
            }
        })
    ...
    # this contains a list of invalid tickers
    invalid_tickers = list(yf_shared._ERRORS.keys())
    if (len(invalid_tickers) > 0):
        # drop columns that are invalid
        close_column = close_column.drop(columns=invalid_tickers)
    ...
    epoch_time_list = (close_column.index.astype(
        np.int64)/1000000).astype(np.int64).tolist()
    ...
    for ticker in (ticker_name_arr):
        ...
        close_price = (
            (close_price/(close_price.iat[0]))*10000).astype('int32')
    ...
    # if some ticker is invalid
    if len(invalid_tickers) > 0:
        responseData['data']['error'] = {
            "code": 404,
            "name": "No data found for tickers",
